...
public class ExposureAssetAllocationBuilderTest {
    ...
    public void testAssetAllocationBuilder_whenSuppliedWithMatchingAlloc_thenAllocationAllocValueReturned() {
        ...
        Map<String, BigDecimal> allocMap = result.getAccountAllocationPercentage();
        List<AllocationByExposureDto> exposureAllocations = result.getAllocations();
        HoldingAllocationByExposureDto holdingAllocation = (HoldingAllocationByExposureDto) exposureAllocations.get(0);
        Assert.assertEquals("assetCode", holdingAllocation.getAssetCode());
        Assert.assertEquals("shareAsset", holdingAllocation.getAssetId());
        Assert.assertEquals("assetName", holdingAllocation.getName());
        Assert.assertEquals(BigDecimal.valueOf(1000), holdingAllocation.getInternalBalance());
        Assert.assertEquals(BigDecimal.ZERO, holdingAllocation.getExternalBalance());
        Assert.assertEquals(false, holdingAllocation.getIsExternal());
        Assert.assertEquals(null, holdingAllocation.getSource());
        Assert.assertEquals("Australian shares", holdingAllocation.getAssetSector());
