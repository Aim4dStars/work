...
public class ClientListApiControllerTest
{
    ...
    @Test
    public void testClientContactDetailsUpdateApi() {
        final String clientId = "12345";
        ClientTxnDto clientDto = buildMockIndividualContactDetailsUpdate(clientId);
        when(clientListDtoService.update(any(ClientTxnDto.class), any(ServiceErrors.class))).thenReturn(clientDto);
        ApiResponse response = clientListApiController.update(clientId, clientDto);
        assertThat(((ClientIdentificationDto)response.getData()).getKey().getClientId(), is(clientId));
    }
    ...
    private ClientTxnDto buildMockIndividualContactDetailsUpdate(String clientId) {
        ClientTxnDto dto = mock(ClientTxnDto.class);
        dto.setKey(new ClientKey(clientId));
        when(dto.getUpdatedAttribute()).thenReturn("emails");
        when(dto.getInvestorTypeUpdated()).thenReturn("individual");
        return dto;
    }
