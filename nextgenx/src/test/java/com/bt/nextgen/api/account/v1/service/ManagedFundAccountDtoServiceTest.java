...
public class ManagedFundAccountDtoServiceTest {
    @InjectMocks
    private ManagedFundAccountDtoServiceImpl managedFundService;
    ...
    private ManagedFundAccountDto mfa = new ManagedFundAccountDto(key, "Cash");
    ...
    public void testUpdate_WhenGivenValiDAccount_ThenMFAReturned() {
        ...
        ManagedFundAccountDto result = managedFundService.update(mfa, serviceErrors);
    ...
    @Test
    public void testFind_WhenFound_ThenMatchingReturned() {
        ServiceErrors serviceErrors = new FailFastErrorsImpl();
        ManagedFundAccountDto result = managedFundService.find(key, serviceErrors);
        Assert.assertEquals(mfa.getKey(), result.getKey());
        Assert.assertEquals(mfa.getDistributionOption(), result.getDistributionOption());
    }

    /*
     * @Test public void testFind_AssetCashOnly_ThenCashOnlyOption() {
     * ServiceErrors serviceErrors = new FailFastErrorsImpl();
     * ManagedFundAccountDto result = managedFundService.find(key,
     * serviceErrors); Assert.assertEquals(1,
     * result.getAvailableDistributionOptions().size());
     * Assert.assertEquals(DistributionMethod.CASH,
     * mfa.getAvailableDistributionOptions().iterator().next()); }
     * 
     * @Test public void testFind_ReinvestOnly_ThenReinvestOption() {
     * ServiceErrors serviceErrors = new FailFastErrorsImpl();
     * ManagedFundAccountDto result = managedFundService.find(key,
     * serviceErrors); Assert.assertEquals(1,
     * result.getAvailableDistributionOptions().size());
     * Assert.assertEquals(DistributionMethod.REINVEST,
     * mfa.getAvailableDistributionOptions().iterator().next()); }
     * 
     * @Test public void testFind_Both_AndNonResident_ThenCashOption() {
     * ServiceErrors serviceErrors = new FailFastErrorsImpl();
     * ManagedFundAccountDto result = managedFundService.find(key,
     * serviceErrors); Assert.assertEquals(1,
     * result.getAvailableDistributionOptions().size());
    ...
    @Test
    public void testFind_Both_AndResident_ThenBothOptions() {
        ServiceErrors serviceErrors = new FailFastErrorsImpl();
        ManagedFundAccountDto result = managedFundService.find(key, serviceErrors);
        Assert.assertEquals(2, result.getAvailableDistributionOptions().size());
        Assert.assertEquals(DistributionMethod.CASH.getDisplayName(), result.getAvailableDistributionOptions().get(0));
        Assert.assertEquals(DistributionMethod.REINVEST.getDisplayName(), result.getAvailableDistributionOptions().get(1));
    }

    @Test
    public void testSearch_WhenAccountSupplied_ThenAllAssetsReturned() {
        ServiceErrors serviceErrors = new FailFastErrorsImpl();
        List<ManagedFundAccountDto> resultList = managedFundService.search(key, serviceErrors);
        Assert.assertEquals(1, resultList.size());
        ManagedFundAccountDto result = resultList.iterator().next();
        Assert.assertEquals(mfa.getKey(), result.getKey());
        Assert.assertEquals(2, result.getAvailableDistributionOptions().size());
        Assert.assertEquals(DistributionMethod.CASH.getDisplayName(), result.getAvailableDistributionOptions().get(0));
        Assert.assertEquals(DistributionMethod.REINVEST.getDisplayName(), result.getAvailableDistributionOptions().get(1));
