...
import com.bt.nextgen.service.wrap.integration.asset.performance.WrapAssetPerformanceIntegrationServiceImpl;
...
public class WrapAccountPerformanceOverallDtoServiceTest {

    @InjectMocks
    private WrapAccountPerformanceOverallDtoServiceImpl performanceOverallService;
    ...
    @Mock
    @Qualifier("WrapAssetPerformanceIntegrationServiceImpl")
    private WrapAssetPerformanceIntegrationServiceImpl wrapAssetPerformanceService;
    ...
    private DateTime wrapEndDate;
    private DateRangeAccountKey key;

    private AccountPerformanceOverall accountPerformanceOverall;
    private AccountPerformanceOverall wrapAccountPerformanceOverall;
    ...
    public void setup() {
        ...
        wrapAccountPerformanceOverall = createWrapPerformanceTestData();
        accountKey = "M00230236";
        startDate = DateTime.parse("2012-02-01");
        DateTime migrationDate = DateTime.parse("2013-02-01");
        endDate = DateTime.parse("2013-03-31");
        wrapEndDate = DateTime.parse("2013-01-31");
        accountDetail.setAccountStructureType(AccountStructureType.SUPER);
        accountDetail.setMigrationDate(migrationDate);
        accountDetail.setMigrationKey(accountKey);
    ...
    @Test
    public void find_whenMethodInvoked_ThenCorrectArgumentsArePassed() {
        when(
                accountService.loadWrapAccountDetail(any(AccountKey.class),any(ServiceErrors.class))).thenReturn(
                accountDetail);

        when(
                accountPerformanceService.loadAccountOverallPerformance(any(AccountKey.class),
                        any(DateTime.class), any(DateTime.class), any(ServiceErrors.class))).thenAnswer(
        ...
        when(
                wrapAssetPerformanceService.loadAccountOverallPerformance(any(WrapAccountDetail.class),
                        any(DateTime.class), any(DateTime.class))).thenAnswer(
                new Answer<AccountPerformanceOverall>() {

                    @Override
                    public AccountPerformanceOverall answer(InvocationOnMock invocation) throws Throwable {
                        Assert.assertEquals(accountDetail, invocation.getArguments()[0]);
                        Assert.assertEquals(startDate, invocation.getArguments()[1]);
                        Assert.assertEquals(wrapEndDate, invocation.getArguments()[2]);
                        return wrapAccountPerformanceOverall;
                    }
                });
        when(
                wrapAssetPerformanceService.combineAssetPerformance(any(AccountPerformanceOverall.class),
                        any(AccountPerformanceOverall.class), any(WrapAccountDetail.class))).thenAnswer(
                new Answer<AccountPerformanceOverall>() {

                    @Override
                    public AccountPerformanceOverall answer(InvocationOnMock invocation) throws Throwable {
                        Assert.assertEquals(accountPerformanceOverall, invocation.getArguments()[0]);
                        Assert.assertEquals(wrapAccountPerformanceOverall, invocation.getArguments()[1]);
                        Assert.assertEquals(accountDetail, invocation.getArguments()[2]);
                        return accountPerformanceOverall;
                    }
                });
    ...
    @Test
    public void find_whenValuesAreFetchedFromIntegration_ThenModelIsConstructed() {
        when(
                accountService.loadWrapAccountDetail(any(AccountKey.class),any(ServiceErrors.class))).thenReturn(
                accountDetail);
        when(
                accountPerformanceService.loadAccountOverallPerformance(any(AccountKey.class),
                        any(DateTime.class), any(DateTime.class), any(ServiceErrors.class))).thenReturn(
                accountPerformanceOverall);
        when(
                wrapAssetPerformanceService.loadAccountOverallPerformance(any(WrapAccountDetail.class),
                        any(DateTime.class), any(DateTime.class))).thenReturn(
                wrapAccountPerformanceOverall);

        accountPerformanceOverall.getInvestmentPerformances()
                .add(wrapAccountPerformanceOverall.getInvestmentPerformances().get(0));
        accountPerformanceOverall.getInvestmentPerformances()
                .add(wrapAccountPerformanceOverall.getInvestmentPerformances().get(1));

        //Test combine service after merged with different codes
        when(
                wrapAssetPerformanceService.combineAssetPerformance(any(AccountPerformanceOverall.class),
                        any(AccountPerformanceOverall.class), any(WrapAccountDetail.class))).thenReturn(
                accountPerformanceOverall);

        AccountPerformanceOverallDto dto = performanceOverallService.find(key, new FailFastErrorsImpl());

        Assert.assertNotNull(dto);
        Assert.assertEquals(5, dto.getInvestmentPerformances().size());
        ...
        PeriodPerformanceDto td1 = dto.getInvestmentPerformances().get(2);
        testWrapTDPerformanceDto(td1);


        ManagedPortfolioPerformanceDto mp = (ManagedPortfolioPerformanceDto) dto.getInvestmentPerformances().get(4);
        testMPPerformanceDto(mp);

        //Test combine service after merged with same code
        accountPerformanceOverall.getInvestmentPerformances()
                .remove(wrapAccountPerformanceOverall.getInvestmentPerformances().get(0));
        when(
                wrapAssetPerformanceService.combineAssetPerformance(any(AccountPerformanceOverall.class),
                        any(AccountPerformanceOverall.class), any(WrapAccountDetail.class))).thenReturn(
                accountPerformanceOverall);

        AccountPerformanceOverallDto dto1 = performanceOverallService.find(key, new FailFastErrorsImpl());
        Assert.assertNotNull(dto1);
        Assert.assertEquals(4, dto1.getInvestmentPerformances().size());
    }

    @Test
    public void find_whenEmptyValuesAreFetchedFromIntegration_ThenEmptyModelIsConstructed() {
        ...
        Assert.assertNotNull(dto1);
        Assert.assertNull(dto1.getInvestmentPerformances());
    }
    @Test
    public void find_whenWrapValuesAreFetchedFromIntegrationAndAvaloqAreEmpty_ThenModelIsConstructed() {
        PortfolioPerformanceOverallImpl accountPerformance = new PortfolioPerformanceOverallImpl();
        ...
        Assert.assertNotNull(dto);
        Assert.assertEquals(2, dto.getInvestmentPerformances().size());
    }
    @Test
    public void prepaymentsAreNotAdded() {
        ...
        when(
                accountService.loadWrapAccountDetail(any(AccountKey.class),any(ServiceErrors.class))).thenReturn(
                accountDetail);
        when(accountPerformanceService.loadAccountOverallPerformance(any(AccountKey.class),
    ...
    @Test
    public void prepaymentsAreNotAddedManagedPortfolio() {
    ...
    @Test
    public void nonPrepaymentsAreAdded() {
    ...
    private AccountPerformanceOverall createWrapPerformanceTestData () {
        AssetImpl wrapTermDepositAsset = new TermDepositAssetImpl();
        wrapTermDepositAsset.setAssetId(null);
        wrapTermDepositAsset.setAssetType(AssetType.TERM_DEPOSIT);
        wrapTermDepositAsset.setAssetCode("WBC1560TD");
        wrapTermDepositAsset.setAssetName("Westpac Term Dpst 94d 20-FEB-2014 3.45%");


        apTD = new AssetPerformanceImpl();
        apTD.setAssetType(AssetType.TERM_DEPOSIT);
        apTD.setContainerType(ContainerType.DIRECT);
        apTD.setAsset(wrapTermDepositAsset);

        AssetImpl mfund = new AssetImpl();
        mfund.setAssetId("4");
        mfund.setAssetName("Managed fund asset");
        mfund.setAssetType(AssetType.MANAGED_FUND);
        mfund.setAssetCode("BHP");

        AssetImpl mPortfolio = new AssetImpl();
        mPortfolio.setAssetId("5");
        mPortfolio.setAssetName("Managed portfolio asset");
        mPortfolio.setAssetType(AssetType.MANAGED_PORTFOLIO);
        mPortfolio.setAssetCode("CBA");

        List<AssetPerformance> investmentPerformance = new ArrayList<>();

        AssetPerformanceImpl wap = new AssetPerformanceImpl();
        wap.setAssetType(AssetType.MANAGED_FUND);
        wap.setName("BT Cash");
        wap.setContainerType(ContainerType.DIRECT);
        wap.setOpeningBalance(BigDecimal.valueOf(100));
        wap.setClosingBalance(BigDecimal.valueOf(101));
        wap.setCapitalReturn(BigDecimal.valueOf(102));
        wap.setIncome(BigDecimal.valueOf(103));
        wap.setPerformanceDollar(BigDecimal.valueOf(104));
        wap.setPerformancePercent(BigDecimal.valueOf(2));
        wap.setPeriodOfDays(105);
        wap.setPurchases(BigDecimal.valueOf(106));
        wap.setSales(BigDecimal.valueOf(107));
        wap.setAsset(mfund);

        investmentPerformance.add(wap);
        investmentPerformance.add(apTD);
    ...
    private void mockTDPresentationService() {
        ...
        when(
                termDepositPresentationService.getTermDepositPresentation(any(AccountKey.class),
                        any(String.class), any(ServiceErrors.class))).thenReturn(tdPres);
    }
    ...
    private void testWrapTDPerformanceDto(PeriodPerformanceDto td) {
        Assert.assertEquals("Westpac Term Dpst 94d 20-FEB-2014 3.45%", td.getName());
    }
