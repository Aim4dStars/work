...
public class TbrModelPortfolioDetailConverterTest {
    ...
    public void setup() {
        ...
        OfferDetail off = Mockito.mock(OfferDetail.class);
        Mockito.when(off.getOfferId()).thenReturn("offerId");
        model.setAccountType(ModelType.INVESTMENT.getCode());
        model.setOfferDetails(Collections.singletonList(off));
    ...
    public void whenToValidateRequestCalled_thenCorrectRequestReturned() {
        ...
        List<Offer> offList = req.getData().getOfferList().getOffer();
        Assert.assertTrue(offList.size() == 1);
        Assert.assertEquals("offerId", offList.get(0).getOfferId().getVal());
    }
    ...
    @Test
    public void test_getOfferListWithNullOffers() {

        model.setOfferDetails(null);
        IpsReq req = modelPortfolioConverter.toSubmitRequest(model);
        Assert.assertNotNull(req);
        Assert.assertTrue(req.getData().getOfferList() == null);

        model.setOfferDetails(new ArrayList<OfferDetail>());
