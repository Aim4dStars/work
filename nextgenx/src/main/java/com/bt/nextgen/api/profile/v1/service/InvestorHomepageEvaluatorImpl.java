...
/**
 * Created by F030695 on 12/07/2016.
 */
@SuppressWarnings({"squid:S1200", "squid:S1226"})
@Service("investorHomepageEvaluatorV1")
public class InvestorHomepageEvaluatorImpl implements InvestorHomepageEvaluator {
    ...
    public void setInvestorHomepageDetails(ProfileDetailsDto profile, Map<AccountKey, WrapAccount> accountMap,
        ...
        if (accountMap.isEmpty()) {
            ...
            if (areAllApplicationsWithdrawn(applications)) {
                ...
                if (profile.getAccountId() != null && HomePageEnum.CLIENT_ACCOUNT_OVERVIEW.toString().equals(profile.getHomePage())) {
                    final ApplicationDocument accountApplication = selectFirst(doneAndPendingApplications,
                            having(on(ApplicationDocument.class).getBpid().getId(), is(EncodedString.toPlainText(profile.getAccountId()))));
                    if (accountApplication != null && accountMap.containsKey(accountApplication.getBpid())) {
                        setAccountLevelHomePage(profile, accountMap.get(accountApplication.getBpid()), serviceErrors);
                    }
                }
            }
        }
    ...
    private InvestorTncDetails getInvestorTncDetails(List<ApplicationDocument> applicationDocuments, UserProfile activeProfile,
                                                     Map<AccountKey, WrapAccount> accountList, boolean isMigratedAccount) {
    ...
    private void setHomePageForInvestorWithSingleAccount(ProfileDetailsDto profile, List<ApplicationDocument> doneAndPendingApplications,
        ...
        final WrapAccountDetail updatedAccount = accountIntegrationService.loadWrapAccountDetail(accountKey, serviceErrors);
        boolean isMigratedAccount = updatedAccount != null && updatedAccount.getMigrationKey() != null;
        InvestorTncDetails tncDetails = getInvestorTncDetails(doneAndPendingApplications, activeProfile, accountMap, isMigratedAccount);
        boolean isEmulatingMigratedUser = isMigratedAccount && profileService.isEmulating();

        profile.setAccountId(EncodedString.fromPlainText(accountKey.getId()).toString());
        if (isAllAccountsActive(accountMap.values())) {
            if (!tncDetails.isApprover() && !tncDetails.isAcceptedTnc() && !isEmulatingMigratedUser) {
                profile.setHomePage(HomePageEnum.NON_APPROVER_TNCS.toString());
            } else {
                setAccountLevelHomePage(profile, accountMap.get(accountKey), serviceErrors);
            }
        } else {
            if (updatedAccount.getAccountStatus() == AccountStatus.ACTIVE || updatedAccount.getAccountStatus() == AccountStatus.CLOSE) {
                setAccountLevelHomePage(profile, updatedAccount, serviceErrors);
            } else {
    ...
    private void setAccountLevelHomePage(ProfileDetailsDto profile, WrapAccount account, ServiceErrors serviceErrors) {
        profile.setCanNavigate(true);
        if (!UserExperience.ASIM.equals(brokerHelperService.getUserExperience(account, serviceErrors))) {
            // For closed accounts, redirecting to account details page
            profile.setHomePage(AccountStatus.CLOSE.equals(account.getAccountStatus()) ?
                    HomePageEnum.ACCOUNT_DETAILS.toString() : HomePageEnum.INVESTOR_EXPERIENCE.toString());
        } else {
