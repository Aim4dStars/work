 ...
 */
@Deprecated
@Service("PaymentLimitDtoServiceV1")
@SuppressWarnings("all")
// Sonar issues to be fixed in V2
public class PaymentLimitDtoServiceImpl implements PaymentLimitDtoService
{
    ...
    public DailyLimitDto updateDailyLimit(DailyLimitDto keyedObject, ServiceErrors serviceErrors) {
        boolean isPaymentLimitUpdated = false;
        try {
            ...
            if(null != payeeDetails.getPayeeLimits()) {
                for (PayeeLimit payeeLimit : payeeDetails.getPayeeLimits()) {
                    if (null!= payeeLimit.getOrderType()&& keyedObject.getPayeeType().equalsIgnoreCase(payeeLimit.getOrderType().name())) {
                        keyedObject.setPreviousAmount(payeeLimit.getLimitAmount());
                    }
                }
            }
            UpdatePaymentLimitRequest updatePaymentLimitRequest = new BilllerReqImpl();
            ...
            updatePaymentLimitRequest.setModificationIdentifier(payeeDetails.getModifierSeqNumber());
            UpdateAccountDetailResponse response = avaloqAccountIntegrationServiceImpl.updatePaymentLimit(
                    updatePaymentLimitRequest, serviceErrors);
