...
import com.bt.nextgen.service.group.customer.groupesb.address.v7.CustomerAddressManagementV7Converter;
...
@Deprecated
public final class CustomerManagementDataV7Converter {

    private static final Logger logger = LoggerFactory.getLogger(CustomerManagementDataV7Converter.class);

    private CustomerManagementDataV7Converter() {
    }
    ...
    public static CustomerData convertResponseInEmailModel(RetrieveDetailsAndArrangementRelationshipsForIPsResponse response) {
        ...
        for (EmailAddressContactMethod emailAddressContactMethod : emailAddressContactMethods) {
            if (null != emailAddressContactMethod && null != emailAddressContactMethod.getHasAddress() && !CustomerPhoneV7Converter.OBSOLETE.equalsIgnoreCase(emailAddressContactMethod.getValidityStatus())) {
    ...
    public static CustomerData convertResponseInAddressModel(RetrieveDetailsAndArrangementRelationshipsForIPsResponse response, CustomerManagementRequest request) {
        ...
        if (request.getInvolvedPartyRoleType().equals(RoleType.INDIVIDUAL)) {
            if (response.getIndividual() != null && response.getIndividual().getHasPostalAddressContactMethod() != null) {
                ...
                if (address != null) {
                    Address returnedAddress = CustomerAddressManagementV7Converter.convertAddressFromResponse(address);
                    // Usage 'R' indicates domicile. Residential and Mailing is considered as same.
                    ((AddressAdapterV7) returnedAddress).setDomicile(true);
                    ((AddressAdapterV7) returnedAddress).setMailingAddress(true);
    ...
    public static PostalAddressContactMethod filterPostalAddressForIndividual(
        ...
        for (PostalAddressContactMethod address : response.getIndividual().getHasPostalAddressContactMethod()) {
            if (!CustomerPhoneV7Converter.OBSOLETE.equalsIgnoreCase(address.getValidityStatus()) && address.getAuditContext().isIsActive() && (
             ...
             * .v7.svc0258.PriorityLevel.PRIMARY) : true)
             */) {
                return address;
            }
        }
    ...
    /**
     * Method to transform the GESB Service Response from Registered State into
     * the domain level CustomerData object
     * 
     * @param response
     * @return customerData
     */
    ...
    public static CustomerData convertResponseInRegistrationStateModel(RetrieveDetailsAndArrangementRelationshipsForIPsResponse response) {
        ...
        if (null != response.getOrganisation() && response.getOrganisation().getHasRegistration() != null &&
            ...
            for (au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement.xsd.retrievedetailsandarrangementrelationshipsforips.
                v7.svc0258.RegistrationArrangement regIdentifier : response.getOrganisation().getHasRegistration()) {
    ...
    private static CustomerRegisteredState populateCustomerRegistrationIdentifiers(
        au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement.xsd.
            retrievedetailsandarrangementrelationshipsforips.v7.svc0258.RegistrationArrangement regIdentifier) {
        ...
        if (regIdentifier != null) {
            ...
            if (null != regIdentifier.getRegistrationIdentifier()) {
                if (!StringUtils.isEmpty(regIdentifier.getRegistrationIdentifier().getRegistrationNumber())) {
                    customerRegisteredState.setRegistrationNumber(regIdentifier.getRegistrationIdentifier().getRegistrationNumber());
                }

                if (regIdentifier.getRegistrationIdentifier().getRegistrationNumberType() != null) {
                    customerRegisteredState.setRegistrationType(regIdentifier.getRegistrationIdentifier().getRegistrationNumberType().toString());
                }
            }
