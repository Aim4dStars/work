 ...
 */
@Deprecated
@Service("addressManagementV7Service")
public class GroupEsbCustomerAddressManagementV7Impl implements CustomerDataManagementIntegrationService {

    private static final Logger logger = LoggerFactory.getLogger(GroupEsbCustomerAddressManagementV7Impl.class);
    ...
    public CustomerData retrieveCustomerInformation(CustomerManagementRequest request, List<String> operationTypes, ServiceErrors serviceErrors) {
        {
            ...
            if (request != null && request.getInvolvedPartyRoleType() != null && !request.getOperationTypes().isEmpty() && request.getCISKey() != null) {
                ...
                return CustomerManagementDataV7Converter.convertResponseInAddressModel(response, request);
            }
        }
    ...
    public boolean updateCustomerInformation(CustomerData updatedData, ServiceErrors serviceErrors) {
        ...
        MaintainIPContactMethodsRequest requestPayload = GroupEsbAddressUpdateRequestV7Builder.createUpdateIPContactMethods(updatedData, cachedResponse);
