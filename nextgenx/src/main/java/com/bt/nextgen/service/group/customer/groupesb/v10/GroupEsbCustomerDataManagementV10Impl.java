package com.bt.nextgen.service.group.customer.groupesb.v10;
 ...
 */
@Service("customerDataManagementV10Service")
public class GroupEsbCustomerDataManagementV10Impl implements CustomerDataManagementIntegrationService {

    private static final Logger LOGGER = LoggerFactory.getLogger(GroupEsbCustomerDataManagementV10Impl.class);
    ...
    private RetrieveDetailsAndArrangementRelationshipsForIPsResponse retrieveCustomerDetailsFromWebservice(
        ...
        try {
            ...
            correlatedResponse = provider.sendWebServiceWithSecurityHeaderAndResponseCallback(userSamlService.getSamlToken(),
                WebServiceProviderConfig.GROUP_ESB_RETRIEVE_CUSTOMER_DETAILS_V10.getConfigName(), requestPayload, serviceErrors);
    ...
    private CustomerData createResultFromServiceResponse(RetrieveDetailsAndArrangementRelationshipsForIPsResponse response,
        ...
        if(response !=null){
	        for (CustomerManagementOperation operation : request.getOperationTypes()) {
	            switch (operation) {
	                case ADDRESS_UPDATE:
	                    data.setAddress(CustomerManagementDataV10Converter.convertResponseInAddressModel(response, request).getAddress());
	                    break;
	                case PREFERRED_NAME_UPDATE:
	                    data.setPreferredName(CustomerManagementDataV10Converter.convertResponseToPreferredNameModel(response).getPreferredName());
	                    break;
	                case CONTACT_DETAILS_UPDATE:
	                    data.setEmails(CustomerManagementDataV10Converter.convertResponseInEmailModel(response).getEmails());
	                    data.setPhoneNumbers(CustomerPhoneV10Converter.convertResponseInPhone(response, request).getPhoneNumbers());
	                    break;
	                case ARRANGEMENTS:
	                ...
	                case INDIVIDUAL_DETAILS:
	                    data.setIndividualDetails(CustomerManagementDataV10Converter.convertResponseToIndividualDetailsModel(response).getIndividualDetails());
	                    break;
	                default:
	                    break;
	            }
       ...
       CustomerData data, ServiceErrors serviceErrors) {
        ...
        if (null != responseData.getArrangementPaginationInstruction() && responseData.getArrangementPaginationInstruction().isIsMoreRecordsAvailable()) {
            ...
            RetrieveDetailsAndArrangementRelationshipsForIPsRequest requestPayload =
                GroupEsbUserDetailsRequestV10Builder.createPaginatedRetrieveDetailsAndArrangementRelationships(request, arrangementPagination);
        ...
        if (CollectionUtils.isNotEmpty(accountList)) {
            for (InvolvedPartyArrangementRole account : accountList) {
                ...
                if (isBankAccountType(westpacProduct)) {
                    bankAccountList.add(CustomerManagementDataV10Converter.convertResponseToBankAccountModel(account, westpacProduct.getName()));
                }
            }
        }
