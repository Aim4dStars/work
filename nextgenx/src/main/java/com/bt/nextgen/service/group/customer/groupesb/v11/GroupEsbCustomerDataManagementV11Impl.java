...
import com.bt.nextgen.service.group.customer.groupesb.taxresidencecountry.v11.CustomerManagementDataConverter;
 ...
 */
@Service("customerDataManagementV11Service")
public class GroupEsbCustomerDataManagementV11Impl implements CustomerDataManagementIntegrationService {

    private static final Logger LOGGER = LoggerFactory.getLogger(GroupEsbCustomerDataManagementV11Impl.class);
    ...
    private CustomerData createResultFromServiceResponse(RetrieveDetailsAndArrangementRelationshipsForIPsResponse response,
        ...
        for (CustomerManagementOperation operation : request.getOperationTypes()) {
            switch (operation) {
                case ADDRESS_UPDATE:
                    data.setAddress(CustomerManagementDataV11Converter.convertResponseInAddressModel(response, request).getAddress());
                    break;
                case PREFERRED_NAME_UPDATE:
                    data.setPreferredName(CustomerManagementDataV11Converter.convertResponseToPreferredNameModel(response).getPreferredName());
                    break;
                case CONTACT_DETAILS_UPDATE:
                    data.setEmails(CustomerManagementDataV11Converter.convertResponseInEmailModel(response).getEmails());
                    data.setPhoneNumbers(CustomerPhoneV11Converter.convertResponseInPhone(response, request).getPhoneNumbers());
                    break;
                case ARRANGEMENTS:
                    paginatedArrangements(response, request, operationTypes, serviceErrors, data);
                    break;
                case INDIVIDUAL_DETAILS:
                    data.setIndividualDetails(CustomerManagementDataV11Converter.convertResponseToIndividualDetailsModel(response).getIndividualDetails());
                    break;
                case TAX_RESIDENCE_COUNTRY_UPDATE:
                    List<RegistrationArrangement> hasForeignRegistration = response.getIndividual().getHasForeignRegistration();
                    data.setTaxResidenceCountries(CustomerManagementDataConverter.getTaxResidenceCountryDetails(hasForeignRegistration));
                    break;
                default:
                    break;
            }
    ...
    private void paginatedArrangements(RetrieveDetailsAndArrangementRelationshipsForIPsResponse response, CustomerManagementRequest request, List<String> operationTypes, ServiceErrors serviceErrors, CustomerData data) {
        if (operationTypes.contains(BANK_ACCOUNT)) {
            if (!serviceErrors.hasErrors())
             ...
             LOGGER.info("Error retrieving customer data: {}", serviceErrors.getErrorList());
        }
    }
       ...
       CustomerData data, ServiceErrors serviceErrors) {
        ...
        if (null != responseData.getArrangementPaginationInstruction() && responseData.getArrangementPaginationInstruction().isIsMoreRecordsAvailable()) {
            ...
            RetrieveDetailsAndArrangementRelationshipsForIPsRequest requestPayload =
                GroupEsbUserDetailsRequestV11Builder.createPaginatedRetrieveDetailsAndArrangementRelationships(request, arrangementPagination);
        ...
        if (CollectionUtils.isNotEmpty(accountList)) {
            for (InvolvedPartyArrangementRole account : accountList) {
                ...
                if (isBankAccountType(westpacProduct)) {
                    bankAccountList.add(CustomerManagementDataV11Converter.convertResponseToBankAccountModel(account, westpacProduct.getName()));
                }
            }
        }
