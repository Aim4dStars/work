...
import com.bt.nextgen.service.group.customer.groupesb.address.v10.CustomerAddressManagementV10Converter;
...
public final class CustomerManagementDataV10Converter {
    ...
    public static CustomerData convertResponseInAddressModel(RetrieveDetailsAndArrangementRelationshipsForIPsResponse response, CustomerManagementRequest request) {
        ...
        if (request.getInvolvedPartyRoleType().equals(RoleType.INDIVIDUAL)) {
            if (response != null && response.getIndividual() != null && response.getIndividual().getHasPostalAddressContactMethod() != null) {
...
retrievedetailsandarrangementrelationshipsforips.v10.svc0258.RegistrationArrangement regIdentifier) {
        ...
        if (regIdentifier != null) {
            ...
            if (null != regIdentifier.getRegistrationIdentifier()) {
                ...
                for (int i = 0; i < regIdentifier.getRegistrationIdentifier().size(); i++) {
                    if (null != regIdentifier.getRegistrationIdentifier().get(i).getRegistrationNumberType()) {
                        if (regIdentifier.getRegistrationIdentifier().get(i).getRegistrationNumberType().equals(RegistrationNumberType.ABN)) {
                            customerRegisteredState.setAbn(regIdentifier.getRegistrationIdentifier().get(i).getRegistrationNumber());
                            customerRegisteredState.setRegistrationType(regIdentifier.getRegistrationIdentifier().get(i).getRegistrationNumberType().name());
                        } else {
                            customerRegisteredState.setRegistrationNumber(regIdentifier.getRegistrationIdentifier().get(i).getRegistrationNumber());
                            customerRegisteredState.setRegistrationType(regIdentifier.getRegistrationIdentifier().get(i).getRegistrationNumberType().name());
                        }
                    }
                }
