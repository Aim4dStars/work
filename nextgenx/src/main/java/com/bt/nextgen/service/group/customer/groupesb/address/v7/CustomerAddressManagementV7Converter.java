...
import au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement.xsd.retrievedetailsandarrangementrelationshipsforips.v7.svc0258.PostalAddress;
...
public final class CustomerAddressManagementV7Converter {

    private CustomerAddressManagementV7Converter() {
    }

    private static final Logger logger = LoggerFactory.getLogger(CustomerAddressManagementV7Converter.class);
    ...
    public static Address convertAddressFromResponse(PostalAddressContactMethod address) {
        ...
        AddressAdapterV7 addressAdapter = null;
        ...
        if (postalAddress instanceof StandardPostalAddress) {
            ...
            addressAdapter = new AddressAdapterV7(standardPostalAddress);
            ...
            addressAdapter = new InternationalAddressV7Adapter(nonStandardPostalAddress);
    ...
    public static au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement.xsd.retrievedetailsandarrangementrelationshipsforips.v7.svc0258.PostalAddressContactMethod convertResponseInAddressModel(RetrieveDetailsAndArrangementRelationshipsForIPsResponse response, CustomerManagementRequest request) {
        if (request.getInvolvedPartyRoleType().equals(RoleType.INDIVIDUAL)) {
            if (response.getIndividual() != null && response.getIndividual().getHasPostalAddressContactMethod() != null) {
                return CustomerManagementDataV7Converter.filterPostalAddressForIndividual(response);
            }
        } else {
            return CustomerManagementDataV7Converter.filterPostalAddressForOrganization(response);
        }
        return null;
    }
}
