 ...
 */
@Service("customerManagementV10Service")
public class GroupEsbCustomerManagementV10Impl implements CustomerManagementIntegrationService {

    private static final Logger LOGGER = LoggerFactory.getLogger(GroupEsbCustomerManagementV10Impl.class);
    ...
    public CustomerRawData retrieveCustomerRawInformation(CustomerManagementRequest request, List<String> operationTypes, ServiceErrors serviceErrors) {
        ...
        GroupEsbUserDetailsRequestV10Builder.addUserSelectedFilters(involvedPartyEntityFilter, operationTypes);
        requestPayload.setInvolvedPartyEntityFilter(involvedPartyEntityFilter);
        gcmAuditRepository.logAuditEntry(userProfileService.getUserId(), WebServiceProviderConfig.GROUP_ESB_RETRIEVE_CUSTOMER_DETAILS_V10.getConfigName(), CustomerRawDataImpl.getJson(requestPayload));
