...
import au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement.xsd.maintainipcontactmethods.v1.svc0418.*;
...
import com.bt.nextgen.service.group.customer.groupesb.phone.CustomerPhoneUpdateConverter;
...
public final class GroupEsbCustomerContactDetailsRequestBuilder {

    private GroupEsbCustomerContactDetailsRequestBuilder() {}

    private static final Logger logger = LoggerFactory.getLogger(GroupEsbCustomerContactDetailsRequestBuilder.class);
    ...
    public static MaintainIPContactMethodsRequest createContactDetailsModificationRequest(
        ...
        Map<String, au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement.xsd.
                retrievedetailsandarrangementrelationshipsforips.v7.svc0258.EmailAddressContactMethod> cachedEmailContexts =
                applyRoleBasedSettings(customerData, cachedResponse, maintainIPContactMethodsRequest);

        int i=0;
        for (Email  email : customerData.getEmails()) {
            if(email instanceof CustomerEmail && ((CustomerEmail) email).getAction() != null) {
                i += 1;
                String rowSetIdentifier = String.format("%010d", i);
                CustomerEmail customerEmail = (CustomerEmail) email;
 ...
 */
/*
 * private static List<PhoneAddressContactMethod>
 * createRequestForPhoneAddress(CustomerData customerData,
 * RetrieveDetailsAndArrangementRelationshipsForIPsResponse cachedResponse){
 * return CustomerPhoneUpdateConverter.convertResponseInPhone(customerData,
 * cachedResponse); }
 * 
 * 
 * private static void requestSettingsForEmailDelete(EmailAddressContactMethod
 * emailAddressContactMethod,
 * au.com.westpac.gn.involvedpartymanagement.services.
 * involvedpartymanagement.xsd.
 * retrievedetailsandarrangementrelationshipsforips.
 * v7.svc0258.EmailAddressContactMethod cachedEmailContexts, String rowSeq,
 * ObjectFactory objectFactory) {
 * 
 * final au.com.westpac.gn.common.xsd.identifiers.v1.ObjectFactory factory = new
 ...
 * if(cachedEmailContexts.getAuditContext() != null &&
 * StringUtils.isNotBlank(cachedEmailContexts
 * .getAuditContext().getVersionNumber())){
 * maintenanceAudiContext.setVersionNumber
 * (cachedEmailContexts.getAuditContext().getVersionNumber()); }
 * maintenanceAudiContext.setIsActive(Boolean.FALSE);
 * emailAddressContactMethod.setAuditContext(maintenanceAudiContext);
 * 
 * ContactMethodIdentifier contactMethodIdentifier =
 * factory.createContactMethodIdentifier();
 * contactMethodIdentifier.setContactMethodId
 * (cachedEmailContexts.getContactMethodIdentifier().getContactMethodId());
 ...
 * emailAddressContactMethod.setStartDate
 * (cachedEmailContexts.getStartDate().getValue()); }
 * emailAddressContactMethod.setEndDate
 * (DateUtil.convertDateInGregorianCalendar(new Date()));
 * emailAddressContactMethod
 * .setValidityStatus(cachedEmailContexts.getValidityStatus());
 ...
 * cachedEmailContexts.getAddressee() && null !=
 * cachedEmailContexts.getAddressee().getFullName()) {
 * involvedPartyName.setFullName
 * (cachedEmailContexts.getAddressee().getFullName());
 * emailAddressContactMethod.setAddressee(involvedPartyName); }
 * 
 * }
 * 
 * private static void requestSettingsForEmailModify(EmailAddressContactMethod
 * emailAddressContactMethod,
 * au.com.westpac.gn.involvedpartymanagement.services.
 * involvedpartymanagement.xsd.
 * retrievedetailsandarrangementrelationshipsforips.
 * v7.svc0258.EmailAddressContactMethod cachedContextMethod, String roSeqNo,
 * ObjectFactory objectFactory) {
 * 
 ...
 * objectFactory.createMaintenanceAuditContext();
 * if(cachedContextMethod.getAuditContext() != null &&
 * StringUtils.isNotBlank(cachedContextMethod
 * .getAuditContext().getVersionNumber())){
 * maintenanceAuditContext.setVersionNumber
 * (cachedContextMethod.getAuditContext().getVersionNumber()); }
 ...
 * factory.createContactMethodIdentifier();
 * contactMethodIdentifier.setContactMethodId
 * (cachedContextMethod.getContactMethodIdentifier().getContactMethodId());
 * emailAddressContactMethod.setInternalIdentifier(contactMethodIdentifier);
 * 
 * emailAddressContactMethod.setStartDate(DateUtil.convertDateInGregorianCalendar
 * (new Date())); emailAddressContactMethod.setValidityStatus("C");
 * emailAddressContactMethod.setIsActive("true");
 * emailAddressContactMethod.setUsageId(cachedContextMethod.getUsage());
 * 
 * InvolvedPartyName involvedPartyName =
 * objectFactory.createInvolvedPartyName(); if (null !=
 * cachedContextMethod.getAddressee() && null !=
 * cachedContextMethod.getAddressee().getFullName()) {
 * involvedPartyName.setFullName
 * (cachedContextMethod.getAddressee().getFullName());
 * emailAddressContactMethod.setAddressee(involvedPartyName); }
 * 
 * Set old values setOldValues(emailAddressContactMethod, cachedContextMethod,
 * objectFactory); }
 * 
 * private static void setOldValues(EmailAddressContactMethod
 * emailAddressContactMethod,
 * au.com.westpac.gn.involvedpartymanagement.services.
 * involvedpartymanagement.xsd.
 * retrievedetailsandarrangementrelationshipsforips.v7.svc0258.
 * EmailAddressContactMethod cachedContextMethod, ObjectFactory objectFactory) {
 * EmailAddressContactMethod oldContext =
 * objectFactory.createEmailAddressContactMethod();
 ...
 * oldContext.getAuditContext().setVersionNumber
 * (cachedContextMethod.getAuditContext().getVersionNumber());
 * oldContext.getAuditContext().setIsActive(Boolean.FALSE); } if (null !=
 * oldContext.getInternalIdentifier()) {
 * oldContext.getInternalIdentifier().setContactMethodId
 * (cachedContextMethod.getContactMethodIdentifier().getContactMethodId()); }
 * oldContext.setValidityStatus("C"); oldContext.setIsActive("true");
 * oldContext.setUsageId(cachedContextMethod.getUsage());
 * 
 * if (null != cachedContextMethod.getAddressee()) { InvolvedPartyName
 * involvedPartyName = objectFactory.createInvolvedPartyName();
 * involvedPartyName
 * .setFullName(cachedContextMethod.getAddressee().getFullName());
 * oldContext.setAddressee(involvedPartyName); } if (null !=
 * cachedContextMethod.getHasAddress()) { EmailAddress emailAddressOld =
 * objectFactory.createEmailAddress();
 * emailAddressOld.setEmailAddress(cachedContextMethod
 * .getHasAddress().getEmailAddress());
 * oldContext.setHasAddress(emailAddressOld); }
 * emailAddressContactMethod.setHasOldValues(oldContext); }
 * 
 * private static void requestSettingsForEmailAdd(EmailAddressContactMethod
 * emailAddressContactMethod,
 * RetrieveDetailsAndArrangementRelationshipsForIPsResponse cachedResponse,
 ...
 * maintenanceAudiContext.setLastUpdateTimestamp
 * (DateUtil.convertDateInGregorianCalendar(new Date()));
 * emailAddressContactMethod.setAuditContext(maintenanceAudiContext);
 * 
 * emailAddressContactMethod.setIsActive("true");
 ...
 * 
 * InvolvedPartyName involvedPartyName =
 * objectFactory.createInvolvedPartyName();
 ...
 * emailAddressContactMethod.setAddressee(involvedPartyName); } }
 * 
 * private static String getFullNameFromCachedResponse(
 * RetrieveDetailsAndArrangementRelationshipsForIPsResponse cachedResponse) {
 * 
 * for
 * (au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement
 * .xsd. retrievedetailsandarrangementrelationshipsforips.v7.svc0258.
 * EmailAddressContactMethod emailAddressContactMethod :
 * cachedResponse.getIndividual().getHasEmailAddressContactMethod()) {
 * 
 * if (null != emailAddressContactMethod.getAddressee() && null !=
 * emailAddressContactMethod.getAddressee().getFullName()) { return
 * emailAddressContactMethod.getAddressee().getFullName(); } }
 * logger.error("Null addressee found for customer"); return null; }
 * 
 * private static Map<String,
 * au.com.westpac.gn.involvedpartymanagement.services.
 * involvedpartymanagement.xsd.
 * retrievedetailsandarrangementrelationshipsforips.
 * v7.svc0258.EmailAddressContactMethod> applyRoleBasedSettings(CustomerData
 * customerData, RetrieveDetailsAndArrangementRelationshipsForIPsResponse
 * cachedResponse, MaintainIPContactMethodsRequest
 * maintainIPContactMethodsRequest) {
 * 
 * Map<String,
 * au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement
 * .xsd. retrievedetailsandarrangementrelationshipsforips.v7.svc0258.
 * EmailAddressContactMethod> cachedEmailContexts = null;
 * 
 * if
 * (RoleType.INDIVIDUAL.equals(customerData.getRequest().getInvolvedPartyRoleType
 * ())) {
 * maintainIPContactMethodsRequest.setInvolvedPartyType(InvolvedPartyType.
 * INDIVIDUAL); cachedEmailContexts =
 * getCachedEmailContextsIntoAMap(cachedResponse
 * .getIndividual().getHasEmailAddressContactMethod()); } else if
 * (RoleType.ORGANISATION
 * .equals(customerData.getRequest().getInvolvedPartyRoleType())) {
 * maintainIPContactMethodsRequest
 * .setInvolvedPartyType(InvolvedPartyType.ORGANISATION); cachedEmailContexts =
 * getCachedEmailContextsIntoAMap
 * (cachedResponse.getOrganisation().getHasEmailAddressContactMethod()); }
 * return cachedEmailContexts; }
 * 
 * private static InvolvedPartyIdentifier
 * getInvolvedPartyIdentifier(CustomerData customerData) { final
 * au.com.westpac.gn.common.xsd.identifiers.v1.ObjectFactory factory = new
 * au.com.westpac.gn.common.xsd.identifiers.v1.ObjectFactory();
 * InvolvedPartyIdentifier involvedPartyIdentifier =
 * factory.createInvolvedPartyIdentifier();
 * involvedPartyIdentifier.setIdentificationScheme
 * (IdentificationScheme.CIS_KEY);
 * involvedPartyIdentifier.setInvolvedPartyId(customerData
 * .getRequest().getCISKey().getId());
 * involvedPartyIdentifier.setSourceSystem(ServiceConstants.UCM); return
 ...
 * involvedpartymanagement.xsd.
 * retrievedetailsandarrangementrelationshipsforips.
 * v7.svc0258.EmailAddressContactMethod> getCachedEmailContextsIntoAMap(
 * List<au.
 * com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement.xsd.
 * retrievedetailsandarrangementrelationshipsforips
 * .v7.svc0258.EmailAddressContactMethod> contextList) {
 * 
 * Map<String,
 * au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement
 * .xsd. retrievedetailsandarrangementrelationshipsforips.v7.svc0258.
 * EmailAddressContactMethod> retrievedEmailsMap = new HashMap<>(); for
 * (au.com.westpac
 * .gn.involvedpartymanagement.services.involvedpartymanagement.xsd.
 * retrievedetailsandarrangementrelationshipsforips
 * .v7.svc0258.EmailAddressContactMethod cachedEmailContexts : contextList) {
 * retrievedEmailsMap.put(cachedEmailContexts.getHasAddress().getEmailAddress(),
 * cachedEmailContexts); } return retrievedEmailsMap; } }
 */