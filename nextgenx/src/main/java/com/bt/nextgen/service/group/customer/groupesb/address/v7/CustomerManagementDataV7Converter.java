        ...
        for (PostalAddressContactMethod address : response.getIndividual().getHasPostalAddressContactMethod()) {
            if (!CustomerPhoneV7Converter.OBSOLETE.equalsIgnoreCase(address.getValidityStatus()) && address.getAuditContext().isIsActive() && (
                ServiceConstants.INDIVIDUAL_ADDRESS_USAGE.equalsIgnoreCase(address.getUsage()) || "R".equalsIgnoreCase(address.getUsage())
                    ...
                    address.getPriorityLevel().equals(au.com.westpac.gn.involvedpartymanagement.services.involvedpartymanagement.xsd
                    .retrievedetailsandarrangementrelationshipsforips.v7.svc0258.PriorityLevel.PRIMARY)
                    : true)*/) {
                return address;
            }
        }
