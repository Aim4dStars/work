package com.bt.nextgen.service.group.customer.groupesb.state.v7;
...
@Service("regStateManagementV7Service")
public class GroupEsbRegStateManagementV7Impl implements CustomerDataManagementIntegrationService {

    private static final Logger logger = LoggerFactory.getLogger(GroupEsbRegStateManagementV7Impl.class);
    ...
    public CustomerData retrieveCustomerInformation(CustomerManagementRequest request, List<String> operationTypes, ServiceErrors errors) {
        ...
        if (request != null && request.getInvolvedPartyRoleType() != null && !request.getOperationTypes().isEmpty() && request.getCISKey() != null) {
            ...
            return CustomerManagementDataV7Converter.convertResponseInRegistrationStateModel(response);
        }
    ...
    public boolean updateCustomerInformation(CustomerData customerData, ServiceErrors serviceErrors) {
        ...
        ModifyOrganisationCustomerRequest requestPayload = GroupEsbRegStateRequestV7Builder.createStateModificationRequest(customerData,cachedResponse);
